...
export function updateThemeUI() {
    ...
    const iconHtml = isLight ? sunSvg : moonSvg;
    $('themeIcon').innerHTML = iconHtml;
    $('themeLbl').textContent = isLight ? 'Темна тема' : 'Світла тема';
    // Sync mobile FAB icon (G.3-mobile-redesign)
    const mobileSvg = document.getElementById('mobileThemeIcon');
    if (mobileSvg) mobileSvg.innerHTML = iconHtml;
}
